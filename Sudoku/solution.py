...
unitlist = unitlist + d1_units + d2_units
...
def eliminate(values):
    ...
    values1 = [i for i in values.keys() if len(values[i]) == 1]
    for key in values1:
        num = values[key]
        for v in peers[key]:
            #values[v] = values[v].replace(num,"")
            values = assign_value(values,v,values[v].replace(num,""))
    return values
...
def only_choice(values):
    ...
    for unit in unitlist:
        for digit in '123456789':
            num = [box for box in unit if digit in values[box]]
            if len(num) == 1:
                #values[num[0]] = digit
                values = assign_value(values,num[0],digit)
    return values
...
'''
for_test_grid = '9.1....8.8.5.7..4.2.4....6...7......5..............83.3..6......9................'
for_test = grid2values(for_test_grid)
display(for_test)
res = eliminate(for_test)
res = only_choice(res)
res = naked_twins(res)
print('\n')
display(res)
print('\n')
display(solve(for_test_grid))
'''
